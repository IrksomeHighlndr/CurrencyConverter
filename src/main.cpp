...
#define Pause_and_Exit {std::cin.ignore(); std::cin.get(); return 0;}
...
int main()
{
    ...
    std::cout << "---------------------------------------" << std::endl;
    std::cout << "Enter the amount and the currency type (SGD, MYR, IDK)";
    std::cin >> Curr1.Amount;
    std::cin >> Curr1.Curr_Type;
    std::cout << "To be converted to (SGD, MYR, IDK):";
    std::cin >> Curr1.To_Be_Curr_Type;
    if (!std::cin)
    {
        std::cout << "\nSeriously?????, Are you drunk? go back to school" << std::endl;
        Pause_and_Exit
    }
    ...
    if (b_Flag)
    {
        std::cout << std::endl << "The converted amount is " << Curr1.Amount << ' ' << Curr1.Curr_Type << std::endl;
        Pause_and_Exit

    }
    std::cout << "Dont out-pizza the hut" << std::endl;
    Pause_and_Exit
}
